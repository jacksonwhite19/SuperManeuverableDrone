...
The optimizer includes a comprehensive remote monitoring system that allows you to monitor and control the optimization process from anywhere, including during long-running optimizations or when you're away from your computer.
...
1. **Generate Dashboard**:
   ...
   ```
   This creates `dashboard.html` - a self-contained, interactive dashboard with real-time metrics.

2. **Start Web Server**:
   ```bash
   python remote_control_server.py
   ```
   Access the dashboard at: `http://localhost:8080` (or your computer's IP address)

3. **Remote Access Options**:
   - **Local Network**: Access from any device on your network using `http://[YOUR_IP]:8080`
   - **Internet Access**: Use ngrok for secure tunneling:
     ```bash
     ngrok http 8080
     ```
   - **Cloud Sync**: If the Optimization folder is in OneDrive/Dropbox, monitor `optimizer_status.json` and `dashboard.html`

### Web Dashboard Features

The web dashboard provides:

- **Real-time Status**: Auto-refreshing display of current optimization state
- **Progress Tracking**: Generation counter, iteration progress, convergence trends
- **Performance Metrics**: Best objective, L/D trends, static margin analysis
- **Differential Evolution Phase Analysis**: Current DE phase (Exploration, Transition, Exploitation, Convergence) with expected behavior
- **Tier Analysis**: Tier 1 (cruise) and Tier 2 (stability) gate status with detailed pass/fail reasons
- **Most Recent Run**: Complete statistics for the latest design evaluation
- **Actionable Insights**: AI-powered analysis of optimization trends and recommendations
- **Alerts & Warnings**: Automatic detection of issues (stagnation, constraint violations, etc.)
- **Interactive Charts**: 
  - Objective function evolution
  - L/D vs Static Margin trade-off
  - Design parameter trends
  - Constraint violation history
  - Stability category breakdown
- **3D Model Viewer**: Visualize the current design geometry in an interactive 3D viewer
- **Control Interface**: Pause, resume, or stop the optimizer remotely

### Monitoring Tools

#### 1. Web Dashboard (Recommended)
- **URL**: `http://[YOUR_IP]:8080` or ngrok URL
- **Features**: Full-featured dashboard with real-time updates, charts, and control buttons
...
- **Update**: Run `python monitor_dashboard.py` periodically to refresh
- **View**: Open in any web browser (works offline)
- **Share**: Can be synced via OneDrive/Dropbox for remote viewing
...
- **Format**: JSON with all current stats (generation, best objective, current design, etc.)
- **Access**: Read via cloud sync or web server

### Remote Control

#### Via Web Interface
1. Open web dashboard at `http://[YOUR_IP]:8080`
2. Use control buttons: "Pause", "Resume", or "Stop"

#### Via Control File (Cloud Sync)
1. Create/edit `optimizer_control.txt` in the Optimization directory
...
#### Overview
- Current generation and iteration
- Best objective value and improvement trend
- Elapsed time and estimated completion
- Current DE phase and expected behavior

#### Most Recent Run
- Complete design parameters
- Performance metrics (L/D, static margin, VSPAero time)
- Tier 1 and Tier 2 gate status with detailed reasons
- Penalty breakdown
...
- Convergence trends
- Design space exploration status
...
#### Charts & Trends
- Objective function evolution over time
- L/D vs Static Margin scatter plot
- Design parameter trends
- Constraint violation history
- Stability category breakdown

#### 3D Model Viewer
- Interactive 3D visualization of current design
- Load `current.vsp3` model on demand
- Rotate, zoom, and pan controls
- Accessible via `/viewer` route

### Files Generated

- **`dashboard.html`** - Standalone dashboard (regenerate with `monitor_dashboard.py`)
- **`optimizer_status.json`** - Real-time status file (updated every iteration)
- **`opt_history.csv`** - Complete optimization history
- **`current.vsp3`** - Current geometry file (for 3D viewer)
- **`current.stl`** - STL export of current model (generated on demand)

### Security Notes

⚠️ **Important**: The web server has no authentication by default.
...
For detailed setup instructions, see `remote_monitoring_guide.md`.
...
├── pitch_stability.vspscript   # Tier 2 stability analysis script
├── update_geom.vspscript        # Geometry update script
│
├── Remote Monitoring/
│   ├── monitor_dashboard.py     # Generate HTML dashboard
│   ├── remote_control_server.py # Web server for remote access
│   ├── viewer.html              # 3D model viewer
│   ├── check_status.py          # Quick status check script
│   ├── check_progress.py        # Detailed progress analysis
│   ├── check_optimizer_alive.py # Health check script
│   ├── control_optimizer.py     # Command-line control interface
│   ├── remote_monitoring_guide.md # Detailed remote monitoring guide
│   └── convert_to_stl.vspscript # STL export script for 3D viewer
│
├── Analysis Tools/
...
└── Generated Files/ (created during optimization)
    ...
    ├── optimizer_status.json   # Real-time status (JSON)
    ├── dashboard.html          # Web dashboard (regenerate with monitor_dashboard.py)
    ...
    └── current.stl             # STL export (generated on demand)
```
